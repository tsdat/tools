...
# TODO: metadata sheet orient as rows instead of columns
# Remove valid_delta from dependent variable metadata, valid_min.
# Color code cells for optional/required properties
# Using "None" causes yaml/numpy barf in the dataset.yaml page
# New lines after each section,
# Example should have dimensions of "time, height", not "height, time"
# dims, then dtype, then attrs

# Auto-insert lat/lon/alt
...
def config_file_converter(fpath):
    ...
    ## Create yaml files
    out: dict[str, dict[str, Any]] = {"attrs": {}, "coords": {}, "data_vars": {}}
    ret: dict[str, Any] = {
        ...
        "readers": {
            ".*": {
                ...
                "parameters": {
                    "read_csv_kwargs": {"header": 0, "index_col": False, "skiprows": 2}
                },
            }
        },
        "coords": {},
    ...
    for i in dv.index:
        ...
        # Dataset yaml setup
        out["data_vars"].update(
            {
                name: {
                    "dims": dv["Dimensions"][i].replace(" ", "").rsplit(","),
                    "dtype": dv["Datatype"][i],
                    "attrs": {"units": unit},
                }
            }
